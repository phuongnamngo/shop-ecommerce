...
it('allows only one checkout to reserve the last unit', function () {
    ...
    $carts = collect([1, 2])->map(function () use ($variant) { $cart = Cart::factory()->create(['status' => 'active']); $cart->items()->create(['product_variant_id' => $variant->id, 'qty' => 1, 'unit_price' => $variant->price]); return $cart; });
    ...
    $barrier = sys_get_temp_dir().'/checkout-'.uniqid(); mkdir($barrier);
    ...
    $deadline = microtime(true) + 10; while ((! file_exists($barrier.'/ready-0') || ! file_exists($barrier.'/ready-1')) && microtime(true) < $deadline) usleep(10000);
    file_put_contents($barrier.'/release', 'go'); $processes->each->wait();
