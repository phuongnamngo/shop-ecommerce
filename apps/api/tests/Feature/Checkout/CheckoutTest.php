...
it('rejects a shipping rate from another method and invalid geo relationships', function () {
    ...
    $payload = ['shipping_address' => ['recipient_name' => 'A', 'phone' => '0900000000', 'province_code' => 'P1', 'district_code' => 'D1', 'ward_code' => 'W1', 'address_line' => '1 Road'], 'shipping_method_id' => $method->id, 'shipping_rate_id' => $rate->id];
...
it('recomputes totals and creates an atomic order reservation', function () {
    ...
    $response = $this->withHeader('X-Cart-Token', $token)->postJson('/api/v1/checkout', [
        ...
        'shipping_method_id' => $method->id, 'shipping_rate_id' => $rate->id,
...
it('rolls back checkout when stock is insufficient', function () {
    ...
    $this->withHeader('X-Cart-Token', $token)->postJson('/api/v1/checkout', ['shipping_address' => ['recipient_name' => 'A', 'phone' => '0900000000', 'province_code' => 'PR', 'district_code' => 'DR', 'ward_code' => 'WR', 'address_line' => 'Road'], 'shipping_method_id' => $method->id, 'shipping_rate_id' => $rate->id])->assertConflict()->assertJsonPath('errors.0.code', 'INVENTORY_INSUFFICIENT_STOCK');
...
it('applies the documented percentage coupon contract', function () {
    ...
    $this->withHeader('X-Cart-Token', $token)->postJson('/api/v1/checkout', ['shipping_address' => ['recipient_name' => 'A', 'phone' => '0900000000', 'province_code' => 'PP', 'district_code' => 'DP', 'ward_code' => 'WP', 'address_line' => 'Road'], 'shipping_method_id' => $method->id, 'shipping_rate_id' => $rate->id, 'coupon_code' => 'TEN'])->assertCreated()->assertJsonPath('data.discount_total', '10000.00')->assertJsonPath('data.grand_total', '100000.00');
